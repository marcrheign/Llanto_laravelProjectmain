    ...
    <section class="overflow-hidden rounded-3xl border border-gray-200 bg-gradient-to-br from-red-900 via-orange-900 to-amber-900 p-8 text-white shadow-2xl dark:border-gray-800">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <h1 class="mt-3 text-3xl font-semibold lg:text-4xl">Deleted Games</h1>
                <p class="mt-4 max-w-2xl text-sm text-red-100">Restore or permanently delete games from your collection.</p>
            </div>
    ...
    <div class="rounded-3xl border border-gray-200 bg-white/90 p-6 shadow-xl dark:border-gray-800 dark:bg-gray-950">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $games->count() }} deleted game(s)</p>
            </div>
        </div>
                    ...
                    @forelse ($games as $game)
                        <tr class="transition hover:bg-gray-50/70 dark:hover:bg-gray-900/60">
                            ...
                            <td class="px-4 py-4">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ $game->genre ?? 'N/A' }}</p>
                            </td>
                            <td class="px-4 py-4 text-gray-600 dark:text-gray-400">{{ $game->platform->name ?? 'N/A' }}</td>
                            <td class="px-4 py-4 text-gray-600 dark:text-gray-400">{{ $game->deleted_at->format('M d, Y H:i') }}</td>
                            <td class="px-4 py-4">
                                <div class="flex items-center justify-end gap-3">
                                    <form method="POST" action="{{ route('games.restore', $game->id) }}" onsubmit="return confirm('Restore this game?');">
                                        @csrf
                                        <button type="submit" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200">
                                            Restore
                                        </button>
                                    </form>
                                    <form method="POST" action="{{ route('games.force-delete', $game->id) }}" onsubmit="return confirm('Permanently delete this game? This action cannot be undone.');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                                Trash bin is empty. No deleted games.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
